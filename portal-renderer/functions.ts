...
import { buildPortalTransaction, submitPortalTransaction } from './transactions'
import { clientAptos } from '@/contract/client-aptos'
...
export async function onPortalButtonPressed(args: onPortalButtonPressedArgs) {
    ...
    switch (button.type) {
        ...
        case 'tx': {
            const task = buildPortalTransaction(clientAptos, {
                module_arguments: button.module_arguments ?? '',
                module_function: button.module_function ?? '',
                user_address: account.accountAddress.toString(),
            }).pipe(
                Effect.flatMap((transaction) => {
                    return submitPortalTransaction(clientAptos, transaction)
                })
            ).pipe(
                Effect.flatMap((transactionHash) => {
                    return Effect.tryPromise({
                        try: async () => {
                            const response = await fetch(button.target ?? button.post_url, {
                                method: 'POST',
                                body: JSON.stringify({ ...JSON.parse(portalPacket), hash: transactionHash })
                            })
                            const new_portal_url = response.url
                            if (!new_portal_url) {
                                throw new Error('No Location header')
                ...
                Effect.flatMap(({
                    new_portal_url, transactionHash
                }) => {
                    console.log("New Portal URL", new_portal_url)
                    return loadAndParsePortalLink({ url: new_portal_url })
                }
